...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *content = buffer;
    while(!feof(file))
    {
        ...
        {
            *length += bytes;
            buffer = realloc(buffer, BYTES * sizeof(BYTE));
        }
    }
    return true;
}
